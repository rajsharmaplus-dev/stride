...
import { Search, FileCheck, History, Users, AlertCircle, CheckCircle2, ArrowRight, XCircle } from 'lucide-react';

export function Governance({ projects, onSelectProject }) {
    const [searchTerm, setSearchTerm] = useState('');

    const allAuditEvents = useMemo(() => (projects || []).flatMap(p => {
    ...
    }).sort((a, b) => new Date(b.date) - new Date(a.date)), [projects]);

    // SEC-03: Wired-up search filter
    const auditEvents = useMemo(() => {
        if (!searchTerm.trim()) return allAuditEvents;
    ...
    }, [allAuditEvents, searchTerm]);

    // SEC-04: Compute real governance stats from actual project data
    const govStats = useMemo(() => {
        const total = (projects || []).length;
        const closed = (projects || []).filter(p => p.status === 'Closed').length;
        const approved = (projects || []).filter(p => ['Active', 'Closed'].includes(p.status)).length;
        const declined = (projects || []).filter(p => p.status === 'Declined').length;
        const uniqueApprovers = new Set(allAuditEvents.filter(e => e.type === 'Approved').map(e => e.user)).size;
        const complianceRate = total > 0 ? Math.round((approved / total) * 100) : 100;
        return { events: allAuditEvents.length, approvers: uniqueApprovers, compliance: complianceRate, breaches: declined };
    }, [projects, allAuditEvents]);
    ...
    return (
        <div className="space-y-8 animate-fade-in pb-20 font-sans">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <GovStat icon={<FileCheck size={18} className="text-emerald-500" />} label="Compliance" value={`${govStats.compliance}%`} status="Live" />
                <GovStat icon={<History size={18} className="text-[#F05A28]" />} label="Events" value={govStats.events} status="All Time" />
                <GovStat icon={<Users size={18} className="text-slate-900" />} label="Approvers" value={govStats.approvers || '—'} status="Active" />
                <GovStat icon={<AlertCircle size={18} className="text-amber-500" />} label="Declined" value={govStats.breaches} status={govStats.breaches === 0 ? 'Perfect' : 'Review'} />
            </div>
            ...
            <section className="gl-card overflow-hidden">
                <div className="px-8 py-5 border-b border-slate-50 bg-slate-50/50 flex items-center justify-between">
                    <h3 className="font-black text-slate-900 uppercase tracking-widest text-[9px] flex items-center gap-2">
                        ...
                        System Audit Trail
                        {searchTerm && <span className="text-slate-400 font-bold normal-case tracking-normal">({auditEvents.length} results)</span>}
                    </h3>
                            ...
                            {auditEvents.map(event => (
                                <tr key={event.id} className="hover:bg-slate-50/50 transition-colors group">
                                    <td className="px-8 py-4">
                                        <div className="flex items-center gap-3">
                                            <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${event.type === 'Approved' ? 'bg-emerald-50 text-emerald-600' : event.type === 'Declined' ? 'bg-red-50 text-red-500' : 'bg-slate-900 text-white'}`}>
                                                {event.type === 'Approved' ? <CheckCircle2 size={14} /> : event.type === 'Declined' ? <XCircle size={14} /> : <History size={14} />}
                                            </div>
                ...
                {auditEvents.length === 0 && (
                    <div className="p-16 text-center">
                        ...
                        <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest">
                            {searchTerm ? 'No matching records found' : 'No signals detected'}
                        </p>
                    </div>
                )}
            </section>
