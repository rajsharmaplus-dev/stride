...
export function SubmissionForm({ user, users = [], onSubmit, onBack, initialData }) {
    ...
    const handleSubmit = async (isDraft) => {
        if (isDraft) {
            ...
            try { await onSubmit(formData, true); } finally { setIsSubmitting(false); }
            return;
        }
        const errs = validate();
        if (Object.keys(errs).length > 0) { setErrors(errs); setShowErrors(true); return; }
        setIsSubmitting(true);
        try { await onSubmit(formData, false); } catch (e) { setIsSubmitting(false); }
    };
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 lg:gap-x-12 gap-y-6">
                         ...
                         <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <FormSelect label="Reporting Manager" name="managerId" value={formData.managerId} onChange={handleChange} options={users.filter(u => u.role !== 'Employee').map(m => ({ value: m.id, label: m.name }))} required />
                            </div>
                    ...
                    <div className="flex items-center gap-4">
                        <div className="flex items-center gap-2">
                            ...
                            <div className="w-12 h-1 bg-slate-200 rounded-full overflow-hidden">
                                <div className="h-full bg-[#F05A28]" style={{ width: '85%' }} />
                            </div>
                        </div>
                    </div>
                </div>
